...
CREATE or REPLACE FUNCTION avg_invoice_line_price
  ...
  IS ave number;
BEGIN
  SELECT ROUND(AVG(unitprice),2) into ave FROM invoiceline;
  RETURN ave;
END;
/
-- use the function
...
SELECT * FROM employee e WHERE e.birthdate > (TO_DATE('31-DEC-68'));
...
CREATE or REPLACE PROCEDURE delete_invoice(ivn IN number)
  AS
BEGIN
  DELETE FROM invoiceline ivl WHERE ivl.invoiceid = ivn;
  DELETE FROM invoice i WHERE i.invoiceid = ivn;
  commit;
END;
/
BEGIN
...
BEGIN
    dbms_output.put_line('A row was inserted into the album table!');
END;
/
...
--ADMINISTRATION (9.0)
  ...
  --BACKUP DATABASE CHINOOK TO DISK "C:\Backup\chinookbackup.bak";